...
st.markdown("Pianifica i tuoi percorsi, garantisci la carica per il giorno dopo e trova le colonnine di qualsiasi operatore.")
...
margine_emergenza_km = st.sidebar.slider("Margine Emergenza Serale (Km)", 10, 100, 30)

st.sidebar.header("3. Preferenze Ricarica")
# Qui l'utente può scegliere l'operatore o lasciare 'Tutte'
provider_pref = st.sidebar.text_input("Operatore preferito (es: Tutte, Enel X, Tesla, Ionity):", "Tutte")

st.sidebar.header("4. Parametri CO2")
co2_termica = st.sidebar.number_input("Emissioni auto benzina media (g/km):", value=120)
...
for g in giorni:
    with st.expander(f"📍 {g}"):
        ...
        dest = c1.text_input(f"Indirizzo Destinazione", placeholder="Es: Porto Antico, Genova", key=f"dest_{g}")
        dist_stimata = c2.number_input("Km (Sola andata)", min_value=0, value=0, key=f"km_{g}")
...
for i in range(len(giorni)):
    ...
    # Check sicurezza: Batteria residua deve coprire (Km Domani + Margine Emergenza)
    ...
    if livello_fine < energia_necessaria_sicurezza:
        azione = "⚡ RICARICA"
        energia_da_reintegrare = capacita_batteria - livello_fine
        costo_ricarica = energia_da_reintegrare * costo_kwh
        totale_costo += costo_ricarica
        
        # Stima tempi ricarica
        min_lenta = int((energia_da_reintegrare / 7) * 60 / 0.9)
        min_fast = int((energia_da_reintegrare / 50) * 60 / 0.9)
        info_ricarica = f"7kW: {min_lenta}m | 50kW: {min_fast}m"
        livello_fine = capacita_batteria 
    
    # Generazione Link Mappe Dinamico
    filtro_ricerca = "stazioni ricarica auto elettriche" if provider_pref.lower() == "tutte" else f"colonnine {provider_pref}"
    query_map = urllib.parse.quote(f"{filtro_ricerca} vicino a {dest_oggi}")
...
# Funzione per rendere il link cliccabile
def make_clickable(link):
    if "Nessuna" in link or "vicino a " in link and link.endswith(" "):
        return "-"
    return f'<a href="{link}" target="_blank">🔍 Trova {provider_pref}</a>'
...
c2.metric("Spesa Energia", f"€ {totale_costo:.2f}")
...
fig = go.Figure(go.Bar(x=periodi, y=valori, marker_color='#2ecc71', text=[f"{v:.0f}kg" for v in valori], textposition='auto'))
